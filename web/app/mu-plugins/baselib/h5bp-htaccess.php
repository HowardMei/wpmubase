...
class ApacheServerConfig {
  const MARKER = 'Apache Server Config';
  const TEXT_DOMAIN = 'Baselib';
  ...
  protected function __construct() {
    ...
    self::$roots_htaccess_file = locate_template(['theme_rewrites.conf', 'theme_htaccess.conf']);
  ...
  public static function setRulesFilters() {
    ...
    self::$rules_filters = apply_filters('baselib/h5bp-htaccess-filters', self::$rules_filters);
  }
  ...
  protected function write() {
    ...
    if (empty($server_config_rules)) {
      ...
      $server_config_rules = apply_filters('baselib/h5bp-htaccess-rules', $server_config_rules);
